    ...
    def insert_module_config(self, data, **kwargs):
        for module_code_entry in iter_entry_points("gn_module", "code"):
            ...
            if module_code in data["DISABLED_MODULES"]:
                continue
            config_schema = load_entry_point(
                module_code_entry.dist, "gn_module", "config_schema"
            )
            config = load_and_validate_toml(
                get_module_config_path(module_code), config_schema
            )
