...
function gn_psql() {
  PGPASSWORD=$user_pg_pass psql -h $db_host -U $user_pg -d $db_name -v ON_ERROR_STOP=ON $* |& tee -a "${LOG_FILE}"
}
...
if [ "$install_usershub_schema" = true ];
 then
    ...
    wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/adds_for_usershub.sql -P tmp/taxhub
...
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/inpn/data_inpn_taxhub.sql -P tmp/taxhub

# sed to replace /tmp/taxhub to ~/<geonature_dir>/tmp.taxhub
sed -i "s#FROM .*/tmp/taxhub\(.*\)'#FROM '${BASE_DIR}/tmp/taxhub\1'#g" tmp/taxhub/data_inpn_taxhub.sql
...
do
  if [ ! -f 'tmp/taxhub/'$i ]
  then
      wget -nc http://geonature.fr/data/inpn/taxonomie/$i -P tmp/taxhub
  else
      echo $i exists
  fi
  unzip -u tmp/taxhub/$i -d tmp/taxhub
done

echo "Getting 'taxonomie' schema creation scripts..."
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/taxhubdb.sql -P tmp/taxhub
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/taxhubdata.sql -P tmp/taxhub
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/taxhubdata_atlas.sql -P tmp/taxhub
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/materialized_views.sql -P tmp/taxhub
wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/generic_drop_and_restore_deps_views.sql -P tmp/taxhub
...
sudo -u postgres -s psql -d $db_name -v ON_ERROR_STOP=ON -f tmp/taxhub/data_inpn_taxhub.sql |& tee -a "${LOG_FILE}"  # FIXME remove sudo
...
wget -nc https://raw.githubusercontent.com/PnX-SI/Habref-api-module/$habref_api_release/src/pypn_habref_api/data/habref.sql -P tmp/habref
wget -nc https://raw.githubusercontent.com/PnX-SI/Habref-api-module/$habref_api_release/src/pypn_habref_api/data/data_inpn_habref.sql -P tmp/habref

# sed to replace /tmp/taxhub to ~/<geonature_dir>/tmp.taxhub
sed -i "s#FROM .*/tmp/habref\(.*\)'#FROM '${BASE_DIR}/tmp/habref\1'#g" tmp/habref/data_inpn_habref.sql
...
gn_psql -f data/core/ref_geo.sql -v MYLOCALSRID=$srid_local

write_log "Creating 'gn_imports' schema..."
gn_psql -f data/core/imports.sql

write_log "Creating 'gn_synthese' schema..."
gn_psql -f data/core/synthese.sql -v MYLOCALSRID=$srid_local
...
gn_psql -f data/core/monitoring.sql -v MYLOCALSRID=$srid_local 
...
write_log "Creating 'gn_sensitivity' schema"
...
then
    ...
    wget -nc https://raw.githubusercontent.com/PnX-SI/TaxHub/$taxhub_release/data/taxhubdata_taxons_example.sql -P tmp/taxhub
...
then
    geonature db upgrade ref_geo_fr_departments -x data-directory=tmp/geonature |& tee -a "${LOG_FILE}"
    geonature db upgrade ref_geo_fr_municipalities -x data-directory=tmp/geonature |& tee -a "${LOG_FILE}"

    # sensibilité apres les dept (pour les liens)
    write_log "Insert 'gn_sensitivity' data ... (This may take a few minutes)"
    geonature db upgrade sensitivity_inpn_rules |& tee -a "${LOG_FILE}"
fi


if [ "$install_grid_layer" = true ];
then
    geonature db upgrade ref_geo_inpn_grids_1 -x data-directory=tmp/geonature |& tee -a "${LOG_FILE}"
    geonature db upgrade ref_geo_inpn_grids_5 -x data-directory=tmp/geonature |& tee -a "${LOG_FILE}"
    geonature db upgrade ref_geo_inpn_grids_10 -x data-directory=tmp/geonature |& tee -a "${LOG_FILE}"
fi
...
if [ "$MODE" = 'dev' ];
then
    echo 'Keep temporary files in dev mode'
else
# Suppression des fichiers : on ne conserve que les fichiers compressés
    echo "Cleaning files..."

    rm -f tmp/geonature/BDALTIV2_250M_FXX_0098_7150_MNT_LAMB93_IGN69.asc
    rm -f tmp/geonature/IGNF_BDALTIr_2-0_ASC_250M_LAMB93_IGN69_FRANCE.html
